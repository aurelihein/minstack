 ...
 * \return 0 if the minstack_tcp stack stopped correctly
 */
int minstack_tcp_write(minstack_tcp *mt, int cid, char *message,
        int len_message) {
    if (!mt) {
    ...
    if (write(cid, message, len_message) < 0) {
        printerror("ERROR writing to socket %d",cid);
        return -1;
    }
...
int minstack_tcp_write_to_server(minstack_tcp *mt, char *message, int len_message) {
    ...
    if (write(mt->listen_socket_fd, message, len_message) < 0) {
        printerror("ERROR writing to server socket %d",mt->listen_socket_fd);
        return -2;
    }
    printmessage("sent:%s length:%d to cid:%d\n",message,len_message,mt->listen_socket_fd);
    return 0;
}
